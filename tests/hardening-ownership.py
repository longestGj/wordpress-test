...
snapshot=wp('eval',"$p=get_page_by_path('resources'); echo json_encode(['id'=>$p->ID,'key'=>get_post_meta($p->ID,'_tio2_hub_key',true),'source'=>get_post_meta($p->ID,'_tio2_owner',true),'front'=>get_option('show_on_front'),'home'=>get_option('page_on_front'),'content'=>$p->post_content]);").stdout
state=json.loads(snapshot)
try:
    for mutation in ["delete_post_meta($p->ID,'_tio2_owner');", "update_post_meta($p->ID,'_tio2_hub_key','foreign');"]:
        wp('eval',"$s=json_decode("+json.dumps(snapshot)+",true); $p=get_post($s['id']); update_post_meta($p->ID,'_tio2_owner',$s['source']); "+mutation+" update_option('show_on_front','posts');")
    ...
    wp('eval',"$s=json_decode("+json.dumps(snapshot)+",true); update_post_meta($s['id'],'_tio2_hub_key',$s['key']); update_post_meta($s['id'],'_tio2_owner',$s['source']); update_option('show_on_front',$s['front']); update_option('page_on_front',$s['home']);")
