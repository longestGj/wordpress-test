...
$id = $post->ID;
$original = ['content'=>$post->post_content, 'source'=>get_post_meta($id,'_tio2_owner',true), 'identity'=>get_post_meta($id,'_tio2_resource_id',true), 'seo'=>get_post_meta($id,'_tio2_seo_title',true)];
...
try {
    ...
    foreach (['_tio2_owner','_tio2_resource_id'] as $field) {
        ...
        update_post_meta($id,$field,wp_slash($field==='_tio2_owner'?$original['source']:$original['identity']));
    }
} finally {
    wp_update_post(['ID'=>$id,'post_content'=>wp_slash($original['content'])]);
    update_post_meta($id,'_tio2_seo_title',wp_slash($original['seo']));
    update_post_meta($id,'_tio2_owner',wp_slash($original['source']));
