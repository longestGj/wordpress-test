...
try{
 ...
 $check(str_contains($notice,'coatings')&&str_contains($notice,'Product relationship consistency'),'Missing taxonomy relationship not warned');
 ...
 ob_start();do_action('admin_notices');$notice=ob_get_clean();
 $check(!str_contains($notice,'Product relationship consistency'),'Disabled or combined copy produced a false relationship warning');
 $data['applications'][0]['enabled']=true;update_post_meta($id,'_tio2_product',$data);
 ob_start();do_action('admin_notices');$notice=ob_get_clean();
 $check(!str_contains($notice,'Product relationship consistency'),'Valid relationship warned');
 ...
 foreach(['m-510','cr-901'] as $slug){$GLOBALS['post']=get_page_by_path($slug,OBJECT,'product');ob_start();do_action('admin_notices');$notice=ob_get_clean();$check(!str_contains($notice,'Product relationship consistency'),'Approved '.$slug.' warned');}
