...
def wp(*args,check=True):
 ...
 return p
snapshot=wp('eval',"$p=get_page_by_path('applications/titanium-dioxide-for-coatings'); echo json_encode(['id'=>$p->ID,'content'=>$p->post_content,'source'=>get_post_meta($p->ID,'_tio2_source',true),'targets'=>get_option('tio2_targets')]);").stdout
...
try:
 ...
 wp('post','meta','delete',str(s['id']),'_tio2_source')
 ...
 wp('post','meta','update',str(s['id']),'_tio2_source',s['source'])
 ...
 wp('eval',"$s=json_decode("+json.dumps(snapshot)+",true); wp_update_post(['ID'=>$s['id'],'post_content'=>wp_slash($s['content'])]); update_post_meta($s['id'],'_tio2_source',$s['source']);")
