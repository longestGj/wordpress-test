...
if(!$parent||get_post_meta($parent->ID,'_tio2_hub_key',true)!=='applications'||$parent->post_status!=='publish')WP_CLI::error('Applications parent ownership/readiness mismatch.');
foreach(glob('/workspace/data/process-applications/*.json') as $file){
    ...
    if($existing&&(get_post_meta($existing->ID,'_tio2_topic',true)!==$s['key']||get_post_meta($existing->ID,'_tio2_source',true)!==$s['source']))WP_CLI::error('Page ownership collision: '.$path);
...
foreach($seeds as $s){
    ...
    foreach(['_tio2_topic'=>$s['key'],'_tio2_source'=>$s['source'],'_tio2_main_class'=>$s['main_class'],'_tio2_seo_title'=>$s['seo_title'],'_tio2_seo_description'=>$s['seo_description']] as $k=>$v)update_post_meta($id,$k,$v);
