...
foreach(glob('/workspace/data/pages/*.json') as $file){
 ...
 if($existing && !tio2_owns_page($existing->ID,'_tio2_hub_key',$s['key']))WP_CLI::error('Page ownership collision at /'.$s['slug'].'/. Existing page was not adopted; no import changes made.');
 $seeds[]=$s;
}
foreach($seeds as $s){
 $existing=get_page_by_path($s['slug'],OBJECT,'page');
 ...
 foreach(['_tio2_owner'=>'tio2-wordpress','_tio2_hub_key'=>$s['key'],'_tio2_main_class'=>$s['main_class'],'_tio2_seo_title'=>$s['seo_title'],'_tio2_seo_description'=>$s['seo_description'],'_tio2_source'=>$s['source']] as $k=>$v)update_post_meta($id,$k,$v);
