...
Main local verification: refinement integrated as 8926c14, localhost:8080 exact migration applied once and second run changed nothing. Full Products HTTP/responsive checks passed again on the main instance. The pre-migration database snapshot is retained under ignored backups/before-products-refinement-20260923.sql. No production publication was performed.

Production publication on 2026-09-23: the user authorized release of the updated Home and Products pages. Source `dfa483fd3b70ea33f47b62ed0552aae422b8636b` was deployed after a verified backup; an exact, guarded Products migration changed six passages and reported no pending changes on repeat runs. Public Products checks passed the 14 grade links and existing summaries, all selector mappings, Not Sure guidance, process links, canonical, ItemList, four FAQs and 1440/768/390 responsive/browser checks. The four grade-summary source-verification gaps above remain open, so this Page Spec stays in REVIEW while the published page is available for user acceptance. See the [production release record](../../docs/audits/production-release-2026-09-23.md).
