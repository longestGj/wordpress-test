...
Explain Malaysia COO and traceability evidence without making unverified shipment claims.

## 内容与事实

- [Current content input](<../inputs/pages/documents/certificate-of-origin/04_planning/DOC-COO_GATE2_FULL_BUYER_CLEAN_COPY_V0.2.md>)
...
- [Visual reference](<../inputs/pages/documents/certificate-of-origin/04_planning/gate5-v0.1/DOC-COO_GATE5_COMPLETE_VISUAL_V0.1.html>)
...
- 按当前正文及视觉输入生成 `data/documents/DOC-COO.json`，正文存原生 Core HTML blocks，可在 Page 编辑器修改；SEO 使用现有可回修字段。来源日期保持原日期。
...
- 在真实 `http://localhost:8080/documents/certificate-of-origin/` 重核批准正文、6 个模块、官方来源与 7 September 2026 的网站复核日期、SEO title/meta/local canonical/noindex、站内路径及 1440/768/390 布局；三种宽度均无整页横向溢出、图片加载正常。全页及首屏截图保存在忽略目录 `.local/document-acceptance/`，文件前缀为 `DOC-COO`。
- 手机菜单 Documents 当前态、Escape 关闭与焦点回返通过；手机证据比较表已调整为逐项卡片并保留表格语义。本页原内容没有 FAQ，因此 FAQ 检查不适用。
- `tests/document-render.php`、`tests/document-browser.py`、`tests/document-http.py`、`tests/document-import.php` 与规划完整性检查通过；接收页 `/request-documents/` 尚为 404，申请动作隐藏且未做表单联调。全站 RFQ `/request-a-quote/` 当前亦为 404，属全站依赖；未执行生产发布。
